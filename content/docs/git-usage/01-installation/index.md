...
(ここからは、Windows 設定のみです。macOS/Linux の人はおそらく設定不要のはずです。[Git 設定](#3-git-設定)に進んでください)
...
<!-- 昔は `master`-`slave` (主人-奴隷) がよく使われていました。 -->

現在では **`main`-`develop` (メイン-開発) が一般的**に利用されています。そのため、上から2番目を選択し、`main` とするのが良いでしょう。
...
# 例
# git config --global user.name happy-coding
